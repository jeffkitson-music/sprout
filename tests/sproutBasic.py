...
def main():
    ...
    while i < 4:
        if len(answers) > 1:
            break
        while True:
            kq = input("Do you have Key {} [y/n]? ".format(i))
            if kq[0].lower() == "y":
                answers.append(i)
                break
            elif kq[0].lower() == "n":
    ...
    print(answers)
    if len(answers) < 2:
        print("Invalid. Not enough keys!")
    keys = []
    for key in answers:
        k = word_input(key)
        keys.append(k)
    print(keys)
    w = str(answers[0])+" & "+str(answers[1])
    s = recover_seed(keys[0],keys[1],w)
    show_seed(s)
...
def show_seed(phrase):
    print()
    print("=== Original Seed Phrase ===")
    i = 1
    complete_phrase = ""
    for word in phrase:
